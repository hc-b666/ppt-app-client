...
export default function Ppt() {
  ...
  const pptId = localStorage.getItem("pptId");
  ...
  useEffect(() => {
    if (pptId) {
      socket.emit("getPresentation", pptId, nickname);

      socket.on("presentationReceived", (res) => {
        if (res.success) {
          setPpt(res.presentation);
        } else {
          console.error("Error getting presentation:", res.error);
        }
      });
    } else {
      navigate("/");
  ...
  useEffect(() => {
    if (ppt) {
      const slide = ppt.slides.find((slide: any) => slide.order === currSlide);
      if (slide) {
        setElements(slide.objects);
      }
    }
  }, [ppt, currSlide]);

  const handleAddSlide = () => {
    socket.emit("addSlide", pptId, nickname);
  ...
  const handleUpdateElements = ()  => {
    socket.emit("updateElements", pptId, currSlide, elements);

    socket.on("elementsUpdated", (res) => {
      console.log(res)
      if (res.success) {
        console.log(res);
      } else {
        console.error("Error updating elements:", res.error);
      }
    });
  };
  ...
  return (
    <main className="overflow-hidden w-full h-screen">
      <div className="w-full p-5 border-b bg-sky-50 flex items-center justify-between fixed z-50">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => onZoom(-0.1)}>zoom out</button>
          <button onClick={() => onZoom(0.1)}>zoom in</button>
          <span>
            {new Intl.NumberFormat("en-GB", { style: "percent" }).format(scale)}
          </span>
          <button onClick={() => setScale(1)}>make it 1</button>
        </div>
      </div>

      <div className="fixed top-20 left-0 w-[300px] h-screen bg-sky-50 z-50 p-5">
        <h3>Presentation Id: {pptId}</h3>

        {nickname === ppt?.creator_nickname && (
          <div className="flex flex-col gap-3">
            <h3>Slides</h3>

            <h4>Current slide {currSlide}</h4>

            <div className="flex flex-col gap-2">
              {ppt?.slides.map((slide: any) => (
                <button
                  onClick={() => setCurrSlide(slide.order)}
                  key={slide._id}
                >
                  Slide {slide.order}
                </button>
      ...
      <div className="fixed top-20 right-0 w-[300px] h-screen bg-sky-50 z-50 p-5">
        <h3>Users</h3>
      </div>
      ...
      {action === "writing" ? (
        <textarea
          ref={textAreaRef}
          onBlur={handleBlur}
          className="fixed text-2xl m-0 p-0 border-0 outline-none overflow-hidden whitespace-pre bg-transparent z-10"
          style={{
            top: selectedElement
